...
if __name__ == '__main__':
    ...
    try:
        ...
        if sys.argv[1] == "servo2":      
            cnt = 3 
            while (cnt != 0):       
                cnt = cnt - 1
                for i in range(50,140,1):   
                    mdev.writeReg(mdev.CMD_SERVO2,numMap(i,0,180,500,2500))
                    time.sleep(0.005)
                for i in range(140,50,-1):  
                    mdev.writeReg(mdev.CMD_SERVO2,numMap(i,0,180,500,2500))
                    time.sleep(0.005)
            mdev.writeReg(mdev.CMD_SERVO2,numMap(90,0,180,500,2500))      
        if sys.argv[1] == "servo3":      
            cnt = 3 
            while (cnt != 0):       
                cnt = cnt - 1
                for i in range(50,140,1):   
                    mdev.writeReg(mdev.CMD_SERVO3,numMap(i,0,180,500,2500))
                    time.sleep(0.005)
                for i in range(140,50,-1):  
                    mdev.writeReg(mdev.CMD_SERVO3,numMap(i,0,180,500,2500))
                    time.sleep(0.005)
            mdev.writeReg(mdev.CMD_SERVO3,numMap(90,0,180,500,2500))  

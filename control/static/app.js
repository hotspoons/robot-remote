...
robotClient = (function () {
    ...
    function getState(){
        ...
        let inputMap = getInputMap();
		let gamepads = navigator.getGamepads();

		// For each controller, show all the button and axis information
		for (let i = 0; i < gamepads.length; i++) {
            // Only support the first gamepad as input for the robot
            if(i > 0){
                continue;
            }
			let gp = gamepads[i];
			...
			for (j = 0; j < gp.buttons.length; j++) {
                if(typeof inputMap.ACTIONS_MAP[j] !== 'undefined'){
                    let button = gp.buttons[j];
                    state.actions[inputMap.ACTIONS_MAP[j]] = button.pressed;
                }
			}
			// Show axis values
            ...
            for(j = 0; j < gp.axes.length; j++){
                if(typeof inputMap.AXIS_MAP[j] !== 'undefined'){
                    let negativeValue = inputMap.AXIS_MAP[j][0];
                    let positiveValue = inputMap.AXIS_MAP[j][1];
                    let value = gp.axes[j];
                   
                    if(value < 0.0){
                        state.vectors[negativeValue] = value;
                    }
                    else if(value > 0.0){
                        state.vectors[positiveValue] = value;
                    }
                }
            }
		}
...
/** All lifted from https://github.com/beejjorgensen/jsgamepad **/
/**
...
(function () {
	...
	function onFrame() {
        ...
        // Send state to back end
        robotClient.sendState();
